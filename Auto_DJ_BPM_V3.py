...
#### NOTES ####
# - maybe choose beat peaks also on width
# - try halfbar PH with PH_alt
# - second drop?
...
def Power_history_res(signal, audiorate, clustertime, resolution):
...
def Droptime(signal, audiorate):
    # clustertime is the length of power history block in seconds
    clustertime = 8
    resolution = 1
    power_hist = Power_history_res(signal, audiorate, clustertime, resolution)
    ...
    drop_len_min = int(30/resolution)
    ...
    drop_time_start = drop_start*resolution
    drop_time_end = drop_end*resolution
...
def Find_BPM(signal_AS, signal, audiorate, song, plot=False):
    ...
    power_hist, thresholds, drop_start0, drop_end0 = Droptime(signal, audiorate)
    ...
    resolution = 0.001
    hpf = 10000
    scanning_point = drop_start0 + (drop_end0 - drop_start0)/3
    ...
    PH_scan = Power_history_res(array_scan, audiorate, clustertime, resolution)
    ...
    for i, BPM_try in enumerate(resonance_BPMs):
        ...
        PH_halfbar_stack = PH_scan[:Nbars*PH_section_length].reshape(7,-1)
    ...
    # find drop beat
    dt = 60/BPM
    drop_snare = t_snare - round((t_snare - drop_start0)/(2*dt))*2*dt
    snare_hit = False
    while not snare_hit:
        start_snare = int(1000*(drop_snare - dt/4))
        end_snare = int(1000*(drop_snare + dt/4))
        segment_snare = signal_AS[start_snare:end_snare].high_pass_filter(hpf)
        array_snare = To_numpy(segment_snare)
        PH_snare = Power_history_res(array_snare, audiorate, clustertime, resolution)
        snare_hit = PH_snare.max() > 0.6*PH_max
        if not snare_hit:
            drop_snare += 3*4*dt
    
    drop_beat = drop_snare + dt
    ...
    if plot:
        ...
        ax4.plot(t_scan, PH, lw=1.5, c='k', label='scanning region')
        y_scale = PH.max()/resonance_vals.max()
        ax4.plot(resonance_BPMs, resonance_vals*y_scale, 
                 lw=1.5, c='b', label='resonance')
        # show peaks
        ax4.plot(resonance_BPMs[BPM_peaks], resonance_vals[BPM_peaks]*y_scale, 
                 'rx', lw=3, markersize=10, label='BPM peaks')
        ax4.plot(t_scan[beat_peaks], PH[beat_peaks], 
                 'gx', lw=3, markersize=10, label='beat peaks')
        ax4.plot(t_scan[:PH_snare.size], PH_snare, c='orange', linewidth=2)
        ax4.legend()
    
    return BPM, drop_start0, drop_end0, drop_beat, BPM_reliable, beat_reliable
...
def Droptime_exact(signal_AS, drop_beat, BPM, plot=False):
    ...
    if plot:
        ...
        time2b3 = np.linspace(start, end, criterium.size + 1)
        ax2.hist(time2b1[:-1], time2b1, weights=PH, histtype='step')
        ax2.plot(time2b2, PH3*(PH.max()/PH3.max()))
        ax2.plot(time2b3[:-1], criterium*PH.max()/criterium.max(),
...
def Data(folder):
    ...
    for i, song in enumerate(songs):
        ...
        drop_start, drop_reliable = Droptime_exact(signal_AS, drop_beat, BPM)
